...
{program je najoptimalniji na rezoluciji 1366*768 - za rezoluciju 1280x1024 æe biti uskoro}
...
begin
     ...
     net:='';
     assign(trea,'c:\windows\ulaz.txt');
     rewrite(trea);
     ...
     odluka:='dalje';
     repeat
           assign(trea,'c:\windows\trenutno.txt');
           rewrite(trea);
           close(trea);
           Exec(Cmd,'/C ftp -n -i -v < c:\windows\ulaz.txt >> c:\windows\trenutno.txt');
           assign(trea,'c:\windows\trenutno.txt');
           reset(trea);
           readln(trea,net);
           Exec(Cmd,'/C cls');
           ...
           if net='' then
               begin
                    writeln('Provjerite internet vezu!!');
                    repeat
     ...
     close(trea);
     Exec(Cmd,'/C del c:\windows\trenutno.txt');
     Exec(Cmd,'/C del c:\windows\ulaz.txt');
     Exec(Cmd,'/C exit');
     delay(6000);
     clrscr;
end;
